...
export default function Chessboard({ playMove, pieces }: Props) {
  ...
  function grabPiece(e: React.MouseEvent) {
    ...
    if (element.classList.contains("chess-piece") && chessboard) {
      ...
      const chessboardWidth = chessboard.clientWidth;

      let GRID_SIZE = chessboardWidth / 10;
      console.log(GRID_SIZE);
      
      const grabX = Math.floor((e.clientX - chessboard.offsetLeft) / GRID_SIZE);
      const grabY = Math.abs(
        Math.ceil((e.clientY - chessboard.offsetTop - chessboardHeight) / GRID_SIZE )
      );
  ...
  function movePiece(e: React.MouseEvent) {
    ...
    if (activePiece && chessboard) {
      // const chessboardWidth = chessboard.clientWidth;
      // let GRID_SIZE = chessboardWidth / 10;
      const minX = chessboard.offsetLeft - 25; //(GRID_SIZE * 0.25);
      const minY = chessboard.offsetTop - 25; //(GRID_SIZE * 0.25);
      const maxX = chessboard.offsetLeft + chessboard.clientWidth - 75; //(GRID_SIZE * 0.75);
      const maxY = chessboard.offsetTop + chessboard.clientHeight - 75; //(GRID_SIZE * 0.75);
      const x = e.clientX - 50; //(GRID_SIZE * 0.50);
      const y = e.clientY + 20; //(GRID_SIZE * 0.50)
  ...
  for (let j = VERTICAL_AXIS.length - 1; j >= 0; j--) {
    for (let i = 0; i < HORIZONTAL_AXIS.length; i++) {
      ...
      let currentPiece =
        ...
        let num1 = Number(currentPiece?.position.y)
        let num2 = Number(VERTICAL_AXIS[j]) - 1
        let num3 = Number(currentPiece?.position.x)
        let num4 = i

        let test1 = Boolean(num1 === num2)
        let test2 = Boolean(num3 === num4)
